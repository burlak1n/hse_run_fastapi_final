...
from app.config import CAPTAIN_ROLE_NAME, settings
...
class UsersDAO(BaseDAO):
    ...
    async def find_one_by_id(
        self, user_id: int, options: list = None
    ) -> Optional[User]:
        """
        ...
        logger.info(
            f"Поиск пользователя с ID {user_id} с {options_count} опциями загрузки."
        )
        try:
    ...
    async def find_user_command_in_event(
        self, user_id: int, event_id: int = 1
    ) -> Optional[Command]:
        """
        Находит команду, в которой состоит пользователь для указанного мероприятия.
        """
        logger.info(
            f"Поиск команды для пользователя {user_id} в мероприятии {event_id}"
        )
        try:
            query = (
                select(Command)
    ...
    async def is_user_captain_in_command(self, user_id: int, command_id: int) -> bool:
        ...
        """
        logger.info(
            f"Проверка, является ли пользователь {user_id} капитаном в команде {command_id}"
        )
        try:
            query = (
                select(CommandsUser)
            ...
            if command_user:
                ...
                logger.info(
                    f"Результат проверки: пользователь {'является' if is_captain else 'не является'} капитаном"
                )
                return is_captain
    ...
    async def create_command_user(self, CommandsUserBase):
        ...
        """
        logger.info(
            f"Создание записи в таблице CommandsUser для команды {CommandsUserBase.command_id} и пользователя {CommandsUserBase.user_id}"
        )
        try:
    ...
    async def get_registrations_by_date(self, role_name: str = None) -> list:
        ...
        """
        logger.info(
            f"Получение статистики регистраций по дням{' для роли: ' + role_name if role_name else ''}"
        )
        try:
            ...
            if role_name:
                query = query.join(Role, self.model.role_id == Role.id).where(
                    Role.name == role_name
                )

            # Группировка и сортировка
            query = query.group_by(func.date(self.model.created_at)).order_by(
                func.date(self.model.created_at)
            )

            # Выполняем запрос
            logger.info("Выполняем запрос для получения регистраций по дням")
            result = await self._session.execute(query)

            logger.info("Запрос выполнен успешно, получаем результаты")
            ...
            for row in registrations_by_date:
                try:
                    ...
                    logger.error(
                        f"Ошибка при форматировании записи: {e}, строка: {row}"
                    )

            logger.info(
                f"Получена статистика регистраций по дням: {len(formatted_result)} записей"
            )
    ...
    async def get_users_by_role(self) -> dict:
        ...
        try:
            ...
            logger.info(
                f"Получена статистика пользователей по ролям: {roles_distribution}"
            )
    ...
    async def get_unusual_name_registrations_by_date(self) -> list:
        ...
        """
        logger.info(
            "Получение статистики регистраций пользователей с необычным ФИО по дням"
        )
        try:
            from datetime import datetime
            ...
            logger.info(
                f"Получена статистика регистраций пользователей с необычным ФИО по дням: {len(formatted_result)} записей"
            )
            return formatted_result
        except Exception as e:
            logger.error(
                f"Ошибка при получении статистики регистраций пользователей с необычным ФИО по дням: {e}"
            )
            return []
        ...
        try:
            ...
            query = (
                select(
                    ...
                    case((Role.name == "captain", true()), else_=false()).label(
                        "is_captain"
                    ),
                )
...
class CommandsDAO(BaseDAO):
    ...
    async def find_all_by_event(self, event_id: int, options: list = None):
        ...
        try:
            ...
            logger.error(
                f"Ошибка SQL при поиске команд для мероприятия {event_id}: {e}"
            )
            raise
        except Exception as e:
    ...
    async def delete_by_id(self, command_id: int):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Ошибка при каскадном удалении команды: {e}, выполняем ручное удаление связей"
                )
                ...
                cu_stmt = select(CommandsUser).where(
                    CommandsUser.command_id == command_id
                )
...
class CommandInviteDAO(BaseDAO):
    ...
    async def create_invite(self, command_id: int, auto_commit: bool = False) -> str:
        ...
        try:
            ...
            if existing_uuid:
                logger.info(
                    f"Приглашение уже существует для команды {command_id}: {existing_uuid}"
                )
                return existing_uuid

            invite_uuid = str(uuid.uuid4())
            invite = CommandInvite(command_id=command_id, invite_uuid=invite_uuid)
            self._session.add(invite)

            if auto_commit:
                await self._session.commit()
                logger.info(
                    f"Создано приглашение {invite_uuid} для команды {command_id} с автокоммитом"
                )
            else:
                # Используем flush чтобы получить ID, но не коммитим транзакцию
                await self._session.flush()
                logger.info(
                    f"Создано приглашение {invite_uuid} для команды {command_id}"
                )
            ...
            if auto_commit:
                await self._session.rollback()
            logger.error(
                f"Ошибка при создании приглашения для команды {command_id}: {e}"
            )
            raise
    ...
    async def get_uuid_by_command_id(self, command_id: int) -> Optional[str]:
        ...
        try:
            query = select(CommandInvite.invite_uuid).filter(
                CommandInvite.command_id == command_id
            )
            result = await self._session.execute(query)
...
class EventsDAO(BaseDAO):
    ...
    async def get_event_id_by_name(self, event_name: str) -> Optional[int]:
        """
        Получает ID события по его имени
        ...
        try:
            ...
            logger.error(
                f"Ошибка при получении ID события по имени '{event_name}': {e}"
            )
            raise
...
class SessionDAO(BaseDAO):
        ...
        if settings.USE_REDIS:
            ...
            try:
                await self.update(
                    ...
                    values=SessionMakeUpdate(
                        is_active=False, expires_at=datetime.now(timezone.utc)
                    ),
                )
    ...
    async def get_session(self, session_token: str):
        ...
        if settings.USE_REDIS:
            ...
            try:
                session = await self.find_one_or_none(
                    filters=SessionGet(token=session_token)
                )
                if not session or not session.is_valid():
                    logger.warning(
                        f"Недействительная или истекшая сессия с токеном {session_token}"
                    )
                    return None
...
class InsidersInfoDAO(BaseDAO):
    ...
    async def create_or_update(
        self, user_id: int, student_organization: str = None, geo_link: str = None
    ):
        """
        Создает или обновляет информацию об инсайдере
        """
        logger.info(
            f"Создание или обновление информации инсайдера для пользователя {user_id}"
        )
        try:
            ...
            if insider_info:
                ...
                await self._session.commit()
                logger.info(
                    f"Информация инсайдера для пользователя {user_id} обновлена"
                )
                return insider_info
            else:
    ...
    async def get_by_user_id(self, user_id: int):
        ...
        try:
            ...
            insider_info = result.scalar_one_or_none()
            logger.info(
                f"Информация инсайдера {'найдена' if insider_info else 'не найдена'}"
            )
            return insider_info
        except Exception as e:
...
class ProgramDAO(BaseDAO):
    ...
    async def add_score(
        self, user_id: int, score: float, comment: str = None
    ) -> Program:
        """
        ...
        try:
            new_record = Program(user_id=user_id, score=score, comment=comment)
    ...
    async def get_total_score(self, user_id: int) -> float:
        ...
        try:
            ...
            query = select(func.sum(self.model.score)).where(
            ...
            logger.error(
                f"Ошибка при получении суммы баллов пользователя {user_id}: {e}"
            )
            return 0
    ...
    async def get_score_history(self, user_id: int) -> list[Program]:
        ...
        try:
            ...
            logger.info(
                f"Получена история баллов пользователя {user_id}: {len(history)} записей"
            )
            return history
        except Exception as e:
            logger.error(
                f"Ошибка при получении истории баллов пользователя {user_id}: {e}"
            )
            return []
...
class UserProfileDAO(BaseDAO):
    ...
    async def create_or_update(self, user_id: int, email: str = None):
        ...
        try:
            ...
            logger.error(
                f"Ошибка при создании/обновлении профиля пользователя {user_id}: {e}"
            )
            await self._session.rollback()
            raise
